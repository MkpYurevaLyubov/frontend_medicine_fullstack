...
const FormRegistration: React.FC = () => {
  ...
  const onChangeUser = (
    ...
    person.login.length >= 6 &&
    isValidatePassword(person.password) &&
    person.password === person.password2
      ...
      <div className="info">
        <Input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            onChangeUser('login', e)
          }
          title="Логин"
          flag={
            (!user.login || !!user.login) &&
            (user.login.length === 0 || user.login.length > 6) &&
            uniqueLogin
          }
        />
        ...
        <InputPassword
          value={user.password}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            onChangeUser('password', e)
          }
          title="Пароль"
          flag={
            (!user.password || !!user.password) &&
            (!user.password || isValidatePassword(user.password))
          }
        />
        ...
        <InputPassword
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            onChangeUser('password2', e)
          }
          title="Повторите пароль"
          flag={
            (!user.password2 || !!user.password2) &&
            (!user.password2 || user.password === user.password2)
          }
        />
        <span>Не меньше 6 символов и прописные лат.буквы</span>
